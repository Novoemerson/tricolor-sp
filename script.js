...
async function fetchRSS(url) {
    ...
    try {
        ...
        if (!data.contents) throw new Error("Conteúdo vazio retornado");
...
function parseRSS(xml) {
    ...
    console.log(`📰 Encontradas ${noticias.length} notícias neste feed`);
    return noticias;
}
...
async function resumirTexto(texto) {
    ...
    try {
        ...
        console.warn("⚠️ Falha no resumo IA:", e.message);
...
async function buscarImagem(termo) {
    ...
    let imagem = await unsplashSearch();
    return imagem || "https://via.placeholder.com/300x200?text=Sem+Imagem";
}
